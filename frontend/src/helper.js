...
export const saveOrUpdate = function (item, $toast, $t, url) {
    if (item.id) {
        return axios.patch(API_URL + `${url}/${item.id}`, item).then(res => {
...
export const lazyParams = function ($refs, filters) {
    return {
        first: 0,
        rows: $refs.dt.rows,
        sortField: 'id',
...
export const toFormData = (obj, form, namespace) => {
    ...
    for (let property in obj) {
        ...
        if (obj.hasOwnProperty(property)) {
            if (obj[property] !== null && obj[property]._d) {
...
export const teacherRoleId = 2
export const studentRoleId = 3
export const managerRoleId = 4

export const activeStudent = 1
export const inactiveStudent = 2
export const deletedStudent = 3
export const inTrialStudent = 4

export function getRoles() {
    return [
        { id: adminRoleId, tag: 'role.admin' },
        { id: teacherRoleId, tag: 'group.teacher' },
        { id: studentRoleId, tag: 'group.student' },
    ]
}
...
export function getRoleUrl(roleId) {
