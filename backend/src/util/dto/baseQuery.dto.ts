export class BaseQueryDto {
  ...
  static fromRequest(request: BaseQueryDto, organization_id: number) {
    let where = {}
          ...
          if (key === 'OR' || key === 'AND') {
            ...
            Object.keys(request.filters[key]).forEach(columnKey => {
              if (request.filters[key][columnKey].value) {
                let item = new Object({})
                item[columnKey] = this.getOperator(columnKey, request.filters[key][columnKey])
                where[key].push(item)
              }
            })
          } else if (key !== 'global') {
            ...
            request.filters[key]['fields'].forEach(columnKey => {
              let item = new Object({})
              item[columnKey] = this.getOperator(columnKey, {matchMode: 'contains', value: request.filters[key].value})
              where['OR'].push(item)
            })
          }
        } else if (Object.keys(request.filters[key]).length > 0 && !Object.keys(request.filters[key]).includes('value')) {
          where[key] = []
          const fields = request.filters[key]
          Object.keys(fields).forEach(fieldKey => {
            let item = new Object({})
            item[fieldKey] = this.getOperator(fieldKey, {matchMode: fields[fieldKey].matchMode, value: fields[fieldKey].value})
            where[key].push(item)
          })
        }
      })
