...
import organizationNotFoundException from "./exeptions/organizationNotFoundException";
...
export async function validateOrganization(req: Request) {
  let url = req.headers.referer
  let urlParts = /^(?:\w+\:\/\/)?([^\/]+)([^\?]*)\??(.*)$/.exec(url);
  let hostname = urlParts[1];
  if (process.env.MODE === localMode) {
    return prisma.organization.findUnique({where: {id: 1}})
  }

  const organization = await prisma.organization.findUnique({where: {domain: hostname}})
  if (!organization) {
